            ...
            ZStack {
                ScrollView {
                    VStack(spacing: 16) { // Add some spacing between elements
                        WeightInput(Exercise: "Deadlift", WeightLeft: defaults.string(forKey: "DeadliftWeightLeft") ?? "", WeightRight: defaults.string(forKey:     "DeadliftWeightRight") ?? "", Weight: defaults.string(forKey: "DeadliftWeight") ?? "", Note: defaults.string(forKey: "DeadliftNote") ?? "")
